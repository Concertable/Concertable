...
1. **Phase 3a is pushed as PR [#378](https://github.com/Concertable/concertable/pull/378) (OPEN, gate green).** Immediate: get it reviewed + merged (full merge-queue E2E runs automatically; no `skip-e2e`; no backend platform-sync). On merge, `publish-fe-packages.yml` republishes all five tiers with the new bare exports — **this republish is the gate that unblocks step 2.**
