...
- Seeding runs via `DevDbInitializer` (`IDevSeeder`, the dev/E2E path) — **not** `ITestSeeder`. If seed state is wrong, fix the dev seeders, and never seed event-sourced/read-model/payout rows directly (see the `seeding` and `seeding` skills, memory `idevseder_not_itestseeder_for_e2e`).
