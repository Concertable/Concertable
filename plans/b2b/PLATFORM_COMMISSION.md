...
| immutable percentage configuration revision referenced by a Payment-issued binding | selected: stores each rate once while separately recording who accepted it, for which obligation and Stripe context |
...
After commitment, calculations load the binding's referenced configuration rather than the
...
booking. The configuration relationship is many bindings to one revision; the binding
...
- [x] Add immutable percentage-only configuration revisions, concurrency-safe configured-revision
  bootstrap, calculation and binding persistence by configuration foreign key.
...
- any number of bindings can share one immutable configuration without copying its terms;
