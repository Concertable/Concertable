...
| [ ] | `docs/polyrepo-ready` | Finish the split the polyrepo cut requires. **Phase 1 shipped:** the plan process (324 lines) is now `standards/process/PLANS.md` + `HANDOFF.md`. Left, one node at a time, each measured on `origin/main`: N1 the 28 skills (3,285 lines), N2 the 36-row route table’s convention, N3 `api/AGENTS.md` (78), N4 `api/ARCHITECTURE.md` + `MICROSERVICES_ARCHITECTURE.md` (587), N5 root `AGENTS.md` (147), N6 `docs/` (554), N7 the `plans/` tree (§4c-gated), then N8 proving one carved service standalone | docs/guidance-restructure |
