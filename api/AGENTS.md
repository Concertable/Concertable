...
- **`AdminProfileEntity` rows.** Written by B2B's `CredentialRegisteredHandler` calling `IAdminModule.GrantIfEligibleAsync` in the same transaction; venue/artist authority lives in Tenant memberships, not manager-profile tables.
