...
- After M1 PRs #942-#945 and their ordered package/image publication are terminal, restack mechanically onto
  the exact landed P4/main, publish the repaired Payment.Client and Payment.Hosting packages plus the dual-port
  Payment.Web image, pin its immutable B2B/Customer digest, revalidate against that baseline, then deliver M4.
...
watermark before publication or delivery. Its M4R1 false-HTTPS and M4R2 mixed-listener findings are repaired by
the restacked commits; any new finding remains owned by M4.
