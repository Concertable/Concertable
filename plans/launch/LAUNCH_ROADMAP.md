...
- [ ] 🟡 **Browser-storage audit + consent correction** — the shipped generic consent banner is not launch-complete: its analytics/marketing choices currently gate no product integration, while necessary Auth/Stripe storage and an unused sidebar cookie were not first established through an evidence-led inventory. Spin off a feature plan that audits anonymous, authenticated and Stripe-checkout browser storage; removes storage and UI/test machinery not justified by active behaviour; classifies and documents what remains with solicitor input; and retains or redesigns consent only where actual non-exempt optional technology requires it. The plan owns the implementation — do not assume the current banner should be preserved or merely refactored.
