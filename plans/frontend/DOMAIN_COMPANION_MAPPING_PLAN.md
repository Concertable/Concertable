...
| User-editable form → request                                           | React Hook Form owns live values; `zodResolver` produces the request passed directly to the mutation. Use a schema transform for restructuring. |
...
| `app/web/b2b/shared/src/features/concerts/hooks/useMyConcert.ts` and `app/shared/src/features/concerts/store/useConcertStore.ts`       | A full `Concert` read is copied into a public editor store and silently stripped to an update request by Zod.                 | Define `UpdateConcertRequest` and `Concert.toUpdateRequest`; let RHF own editor state inside `useMyConcert` and delete the editor store.                                                                                                                                                                                                       |
| `app/shared/src/features/artists/api/artistApi.ts`, `hooks/useMyArtist.ts`, and `store/useArtistStore.ts`                              | Create types live in the API module; update accepts a full `Artist` read, then manually encodes selected fields as multipart. | Define slim create/update request interfaces and `Artist.toUpdateRequest`; use RHF plus Zod and delete the editor store. Keep module-private multipart encoders in `artistApi.ts`.                                                                                                                                                             |
| `app/shared/src/features/venues/api/venueApi.ts`, `hooks/useMyVenue.ts`, and `store/useVenueStore.ts`                                  | Update accepts a full `Venue` read, then manually encodes selected fields as multipart.                                       | Define slim create/update request interfaces and `Venue.toUpdateRequest`; use RHF plus Zod and delete the editor store. Keep module-private multipart encoders in `venueApi.ts`.                                                                                                                                                              |
...
| Artist, Venue, and Concert editor pages/components                 | Public stores expose form fields and transitions directly; several pages combine those stores with existing workflow hooks.                      | Move editable state into RHF-owned workflow hooks/forms, add create workflows where needed, pass image callbacks into Hero components, and delete editor stores and their exports.                                                                                               |
...
4. Replace the full-Concert edit copy with `Concert.toUpdateRequest` and RHF-owned editor state.
5. Delete `useConcertStore`, remove every hand-written `XBuffer` form abstraction, and make affected
   facade hooks accept parsed request contracts only.
...
3. Add `Artist.toUpdateRequest` and `Venue.toUpdateRequest`; use RHF and aligned Zod schemas so
   mutations receive parsed slim requests without editor buffer types or stores.
4. Keep PascalCase multipart keys, optional image handling, and indexed genre fields inside the API
   modules.
5. Make the Artist and Venue workflow hooks own RHF editor transitions; add create workflows, pass image
   callbacks into Hero components, and delete editor stores and their imports.
...
Gate: Artist/Venue APIs accept no read type, multipart wire behaviour is unchanged, editor stores are
private, and universal shared tests/build plus both mobile builds are green.
...
- React Hook Form owns interactive form state and `zodResolver` produces mutation requests; no
  hand-written `XBuffer`/`XDraft` form abstraction remains.
