...
- Worktree: `C:\Users\TommySeery\source\repos\Concertable.worktrees\Feature\platform_polyrepo_carved-css` — created off current `origin/main` for the carved-web CSS work. The prior `platform_polyrepo-fullstack` worktree was **pruned** after Phase 3b's `Docs/polyrepo_3b-closeout` merged (expected cleanup — merged branch). The remaining Phase 3 items (mobile retarget, ESLint boundary) each start on their own fresh branch off `origin/main`.
- Branch: `Feature/platform_polyrepo_carved-css`, off `origin/main` @ `30c74a844` (0 behind). Carries the one-file carved-web CSS fix (`app/web/shared/src/index.css`).
- PR: **carved-web CSS PR — being opened** (base `main`, no skip labels; `app/**` package-source change → full E2E + republish on merge). **Phase 3b PR [#389](https://github.com/Concertable/concertable/pull/389) MERGED** (`1cbeb2175`) + closeout **[#398] MERGED**. Phase 3a [#378] MERGED (`fba490e25`); Phase 2 [#360] MERGED (`a3f9535`); Phase 1 [#301] + [#319] earlier.
- Dependency/package gates: the fix edits `@concertable/web` package source → on merge, `publish-fe-packages` republishes all 5 tiers with the fixed `index.css`; **the fix becomes effective in carved surfaces on republish** (per "tier change → effective on republish"). No `api/**` → no backend platform-sync. (Platform-sync **#402 merged during this session** — another PR's api sync, not ours.)
- Last reconciled: 2026-08-06 — reconciled to post-#398/#402 `origin/main`; **implemented + carve-proved** the carved-web CSS strategy (before/after generated-CSS inspection); all four web builds green. Next: open the PR, then the remaining Phase 3 items.
...
The carved-web CSS strategy is delivered pending PR/merge/republish (see Current state). Remaining Phase 3 items — start each on a **fresh branch off current `origin/main`** (fetch first):
