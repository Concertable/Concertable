...
| copy version, currency and rate into every binding | rejected: repeats identical configuration terms and conflates a pricing revision with a payer commitment |
...
Replace the flat-fee setting with validated append-only Payment configuration revisions:

```text
PlatformCommission
  CurrentConfigurationId  required configured revision ID
  Configurations
    Id                     required unique non-empty Guid
...
deal-type selector, expiry or mutable status. The revisions bind through normal .NET configuration:
local appsettings and deployed Azure App Configuration expose the same shape. Payment validates the
complete revision set on startup and holds it through `IOptions`; commission rates do not hot-reload inside
a running process.

Changing the rate appends a new ID and version, retains every revision referenced by a live binding,
and moves `CurrentConfigurationId` in the same reviewed configuration change. During a rolling
...
mutation, forcing a refresh. After commitment, calculations resolve the binding's referenced revision
from the configured revisions rather than the current revision.
...
CommissionConfigurationId  required reference into the immutable Payment configuration revisions
...
booking. Many bindings can reference one revision; the binding does not duplicate version, currency
or rate, and Payment does not copy deployment configuration into its database.
...
- [x] Add validated append-only percentage configuration revisions, calculation and binding
  persistence by configuration ID only.
...
- [x] Resolve review finding OWN1: keep the append-only configuration revisions in validated immutable
...
- the current configuration ID must resolve to one validated revision;
- duplicate IDs or versions and invalid terms fail startup, and referenced revisions remain in the
  append-only configuration revision set;
