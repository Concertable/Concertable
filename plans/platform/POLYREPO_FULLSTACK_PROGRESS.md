...
- PR: **Phase 3a PR [#378](https://github.com/Concertable/concertable/pull/378) MERGED** (`fba490e25`, 2026-08-05) — full merge-queue E2E green; remote branch auto-deleted. Phase 2 PR [#360] MERGED (`a3f9535`); Phase 1 [#301] + review-fix [#319] merged earlier. Phase 3b PR: opening now.
...
- Last reconciled: 2026-08-05 (resume) — Phase 3a merged (`fba490e25`) + tiers republished green; **Phase 3b (carve-fe-web CI + `run_fe` gate) authored on the new branch** and being opened as its own PR.

## Current state

Phases 0, 1, 2, and **3a are on `main`**. Phase 3a (PR #378, merge `fba490e25`) landed the tier-subpath rename (`@concertable/<tier>/<path>`, no `/shared`) + surface dependency-closure self-declaration + the `carve-fe.mjs` harness; a mid-air collision with the cookie-consent PR #377 (all four `main.tsx`) was resolved en route. `publish-fe-packages.yml` then republished all five tiers with the bare exports (from-feed verify green). Phases 0–3a are fully terminal.

**Phase 3b is authored on `Feature/platform_polyrepo_carve-fe-ci` (test.yml only), being opened as a PR.** Three coordinated `.github/workflows/test.yml` changes: (1) a `run_fe` classifier output — true iff a non-inert `app/` change OR the carve mechanism itself (`carve-fe.mjs` / `test.yml`) changes, so BE-only PRs skip the slow feed carves while a change to the gate self-validates; (2) a `carve-fe-web` matrix job over the 4 web surfaces, each running `node scripts/carve-fe.mjs <surface>` (checkout + node 20 + `GITHUB_PACKAGES_TOKEN` from `GITHUB_TOKEN`, `packages: read`); (3) `carve-fe-web` wired into `ci-complete`'s required needs. YAML validated; `run_fe` logic unit-tested across 7 file-set cases. Because the PR changes `test.yml`, `run_fe=true` on it → **carve-fe-web runs and validates itself on this very PR** (local carve couldn't finish in the ~9-min shell window; CI is the validation).

## Next Steps

1. **Phase 3b PR (`carve-fe-web` + `run_fe`) — opening now.** Review it (code PR, `test.yml`), then merge via the queue. `carve-fe-web` runs on the PR itself (test.yml change → `run_fe=true`) over the 4 web surfaces restoring the republished bare-export tiers from the feed and building standalone — **that run is the validation.** If a surface's declared closure is incomplete or a specifier misresolves, its carve goes red → fix on the PR (that's exactly the gap the carve exists to catch). No backend platform-sync (app/CI only).
2. **Carved-web CSS `@source` content strategy** — carve BUILD is green, but `@concertable/web`'s `index.css` `@source` globs point at sibling-surface source (`../../customer/src`, `../../b2b/*`) that resolve only in-monorepo; a carved surface generates its own classes (Tailwind v4 auto-detect) but NOT the shared tiers' (their class strings live in `node_modules/@concertable/*/dist`, which auto-detect excludes). A single relative `@source` set can't serve both layouts (monorepo `app/{shared,web/shared,…}` ≠ node_modules `@concertable/*`). Needs a cross-context strategy proven by a carved vite build's generated CSS; tier change → effective on republish.
3. **Mobile metro/nativewind/tailwind retarget** off `../shared` onto `@concertable/mobile` (`watchFolders`, nativewind `input`, tailwind `content`), proven by `expo export` on the precompiled dist; then add `mobile/{customer,b2b}` to the `carve-fe-web` matrix (or a sibling `carve-fe-mobile`).
4. **FE import-boundary rule** — no ESLint/dependency-cruiser toolchain in `app/` yet; the carve CI is the primary structural boundary today (BE parity: carve = structural gate, build-time guard = fast second layer). Standing up ESLint `no-restricted-imports` across surfaces is a separate sub-project.

Gate: `carve-fe-web` green in CI on the Phase 3b PR (step 1).
