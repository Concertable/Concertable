...
- [ ] 🟠 **Manager front-page dashboards.** `launch/manager-front-page` Venue and artist manager workbenches are implemented through the current fixture-backed and KPI slices; remaining UX freeze, real-data wiring, delivery, and closeout are owned by [MANAGER_FRONT_PAGE_PLAN.md](MANAGER_FRONT_PAGE_PLAN.md).
...
- [x] ✅ **API rate limiting** `launch/rate-limiting` — shipped via `Concertable.ServiceDefaults` seam (`AddDefaultRateLimiting`/`UseDefaultRateLimiting`, four named policies, 429 + `Retry-After`), applied in Auth (Login on the `Pages/Account` credential POSTs) and B2B (Apply→`ApplicationController.Apply`, Upload→`BlobController.Upload`, Messaging→`MessageController.Report`). Two evidence-based deviations from the presumed surface: `/connect/token` is left on the generous global limiter, not Login — ROPC is E2E-only, so in production it carries service client-credentials + code exchange, and the tight per-IP Login cap would throttle the platform's own service auth; and there is no user-facing message-compose endpoint (messages are lifecycle-generated via `ApplicationNotifier`, already bounded by Apply), so Messaging protects the one spammable conversations write, `Report`. Distributed-store correctness under horizontal scale is deferred (`api/TECH_DEBT.md`); production per-IP correctness depends on `launch/config-and-deployment` wiring trusted-proxy `ForwardedHeaders`.
