...
- [ ] 🔴 **Organiser direct offers — the second booking entry direction** `launch/booking-entry-direct-offers` — **BLOCKED on the configurable-deal refactor; do not build.** Design pass complete 2026-09-07 and then partly disproven the same day. Entry commitment and binding operation are two axes, not one: DoorSplit-offer needs `MethodVerification` at entry while its binding operation stays `VerifyPayment`, the same value DoorSplit-apply pairs with `None`, so entry is not derivable from binding. Building the two keyed enum axes that follow would build precisely what configuration-backed capability selection deletes, so the ordering is reversed: **configurable deals first, `DealType` removed entirely, direct offers as its first consumer.** Load-bearing prerequisite: `KeyedStrategyBuilder<TKey> where TKey : struct, Enum` cannot express a capability registry identity plus version. First open question: the owner's Route B skips the opportunity entirely, so `Application.OpportunityId` — non-null, and the column both unique indexes are built on — has nothing to hold; extracting the bookable slot from Opportunity is the live candidate. Surviving input: the payer axis (credited to `DIVERGENCE_BLIND_DESIGN.md`), the binding-operation table, the proposal-thread model, the dispatch-site inventory and the route/action-link surface. Payment needs no change either way. Plan: [BOOKING_ENTRY_DIRECT_OFFERS_PLAN.md](BOOKING_ENTRY_DIRECT_OFFERS_PLAN.md) §0.
