...
4. ~~**Dismantle `Modules/User/` TPH.**~~ **DONE `cd872fad`, completed by `Feature/RetireRoleClaim`.** B2B now has a role-agnostic `UserEntity`, tenant memberships for venue/artist authority, and the surviving orthogonal `AdminProfileEntity`; Customer owns its local user projection. The transitional venue/artist manager-profile tables and polymorphic user mappers are gone.
5. ~~**Auth becomes identity-only for B2B.**~~ **DONE `4aa7e641`, completed by `Feature/RetireRoleClaim`.** Auth owns credentials; B2B tokens carry identity only and B2B resolves authority from active tenant membership. Customer's transitional claims remain Customer-owned. `CredentialRegisteredEvent` drives each service's local projection.
