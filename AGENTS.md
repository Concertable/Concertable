...
- **Backend (.NET, `api/`)** — the floor and which skills apply: [`api/AGENTS.md`](./api/AGENTS.md). This system's own rosters are the `concertable-*` skills from `Concertable/agent-standards` — `concertable-persistence` (the `Concertable.DataAccess` capability hierarchy), `concertable-seeding` (the forbidden-table inventory), `concertable-packages` (Reunion pins, the carve gates), `concertable-http-clients` (the Refit inventory), `concertable-microservice-boundaries` (the service roster).
- **Frontend (React/TS, `app/`)** — the tier map: [`app/AGENTS.md`](./app/AGENTS.md). This system's own rosters are `concertable-http-layer` (the four clients, the `isApiError` seam), `concertable-identity` (`User` vs `B2bIdentity`), `concertable-permissions` (`SharedPermissions`) and `concertable-typescript-style` (the live `$type` unions).
