...
description: The design rationale and decision history behind Concertable's event-driven microservices split — why B2B (venue↔artist booking + settlement) and Customer (ticket marketplace) are separate bounded contexts served by a centralised Payment adapter, the data-ownership and projection map, the transactional-outbox→inbox CQRS flow in both directions (Venue-change and ticket-purchase), why the outbox earns its keep against the dual-write problem, service-to-service `client_credentials` auth, Payment's PCI-scope containment, static reference data as contracts, Email as a shared library, the modular monolith kept inside each service, the learning sequence the build follows, the risks/open questions, and the full decision log. This records why the system is shaped the way it is; the current-state rules an agent must follow are the `microservice-boundaries` skill. Use when you need the reasoning behind a cross-service design, when designing an event flow or projection, when adding the first post-commit handler, or when making an architecture decision that spans more than one service.
---

# Concertable Microservices Architecture

> **This is the design and decision record, not the current-state rulebook.** The rules an agent must
> follow — the service roster, adapter-vs-data, what may `WaitFor` what, the surfaces each service exposes,
> shared-code-is-the-intersection — are the `microservice-boundaries` skill; the outbox/inbox mechanism is
> `domain-events`; the package closure and the publish→sync loop are `packages`. This doc explains **why**
> the system is shaped that way and preserves the decisions that got it there.
>
> **Goal:** Migrate from a modular monolith to an event-driven microservices architecture that separates B2B (venue ↔ artist booking + settlement) from Customer (ticket marketplace), with a centralised Payment adapter service.
>
> **Constraint that changed the plan:** This began as a learning side project, and skill development (event-driven architecture, transactional outbox, sagas, OpenTelemetry, Service Bus operations) is an explicit goal alongside any eventual deployment — which is why the learning sequence in §9 is part of the design, not incidental.
...
**Status (2026-05-23):** Currently implemented as `ConcertReadModel` (read-only projection, no `AvailableTickets`). Behavioural shape with `DecrementAvailability` and ticket-capacity ownership is pending — see the Customer service's `TECH_DEBT.md`.
...
- **gRPC between data services** (`B2B`, `Customer`, `Search`) — same antipattern risk as sync HTTP.
  ...
  sync calls — see the `microservice-boundaries` skill.
...
> **Not yet implemented (2026-05-23).** Customer's `TicketEntity.Create` does not raise `TicketPurchasedDomainEvent`; no `TicketPurchasedEvent` integration event exists; B2B has no `ConcertSalesProjection`. Tracked in the Customer and B2B services' `TECH_DEBT.md` files.
...
- **No native mobile apps in launch scope.**
...
- B2B Workers uses `AddInMemoryTransport` (bug — see the B2B service's `TECH_DEBT.md`).
- Inverse-direction event flow (§6) not yet implemented (see the per-service `TECH_DEBT.md` files).
- Per-service `ARCHITECTURE.md` and `TECH_DEBT.md` added at the B2B and Customer services.

**2026-06-05** — Doc-vs-reality reconciliation (big-review NOTE4). The current-state roster and rules are the `microservice-boundaries` skill; this doc records the design and its decision history.
