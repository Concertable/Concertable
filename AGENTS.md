...
- **Frontend (React/TS, `app/`)** — the tier map: [`app/AGENTS.md`](./app/AGENTS.md). This system's own rosters are `http-layer` (the four clients, the `isApiError` seam), `identity` (`User` vs `B2bIdentity`), `permissions` (`SharedPermissions`) and `typescript-style` (the live `$type` unions).
