...
**The generic .NET and TypeScript/React standards are load-on-demand skills, not files in this repo.** The task you are doing is the trigger to read the matching one — `csharp-style`, `persistence`, `multitenancy`, `keyed-strategies`, `result-carriers`, `seeding`, `unit-testing`, `react-structure`, `server-state`, `http-layer`, `tiered-shared-code`, and the rest. The in-repo docs below carry only what those skills deliberately omit: the roster of real types, contexts, clients and tables in *this* system.

- **Backend (.NET, `api/`)** — seeding inventory, migrations, DTOs, module rules, which skills apply: [`api/AGENTS.md`](./api/AGENTS.md), anchoring [`api/agents/CODE_CONVENTIONS.md`](./api/agents/CODE_CONVENTIONS.md) (the `Concertable.DataAccess` capability hierarchy, `IGeometryProvider`), [`api/agents/CODE_PATTERNS.md`](./api/agents/CODE_PATTERNS.md) (the Refit inventory, one repository per entity) and [`api/agents/RESULT_PATTERN.md`](./api/agents/RESULT_PATTERN.md) (Reunion pins and legacy-carrier migration state).
- **Frontend (React/TS, `app/`)** — tiers and precedents: [`app/AGENTS.md`](./app/AGENTS.md), anchoring [`app/agents/CODE_CONVENTIONS.md`](./app/agents/CODE_CONVENTIONS.md) (the four HTTP clients, the `isApiError` seam) and [`app/agents/CODE_PATTERNS.md`](./app/agents/CODE_PATTERNS.md) (`User` vs `B2bIdentity`, `SharedPermissions`).
...
**Working docs and plans are exempt from branch hygiene.** Non-code working markdown — `plans/*.md`,
any `TECH_DEBT.md`, scratch notes — is non-breaking and may ride its owning feature branch. Durable
global guidance is different: when feature work changes an `AGENTS.md`, an `agents/*.md` playbook, a
skill, or another cross-cutting instruction, split that change immediately onto a `Docs/*` branch from
`origin/main`, review it, and land it through `/merge-docs`. Never leave guidance stranded behind a
feature PR or mixed into a runtime commit.
