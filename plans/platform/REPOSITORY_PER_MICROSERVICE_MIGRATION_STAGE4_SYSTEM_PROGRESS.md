...
- Dependency/package gates: service TestKit and system source-factory boundary landed in PR #882
...
service repositories and moving full-system composition into `Concertable/system`. PR #882 established B2B,
Customer, and Payment TestKits, E2E admin hosts, the system AppHost factory, and the carve-E2E mechanism. It did not finish
...
E2E cross-target edges, all owned by the non-packable, carve-excluded system source factory: two each for B2B,
...
behind `ISystemAppHostFactory`. The two previously implicit hard-coded Search implementation paths are now
honest inventory edges in the system source factory, while package mode builds without Search service source.
...
After this slice merges, the next smallest independently shippable system slice
is the Payment pair: replace the system source factory's Payment Web and Workers implementation references
...
- PR #882 merged as `b1a754ef9`, establishing service-owned TestKits, E2E admin hosts, system composition contracts, and `carve-e2e.ps1`.
- The system source and package-mode factory seam is present on current main.
- Created the owning isolated worktree and branch from `origin/main` `cf0da4c9b`.
- Merged current `origin/main` `67a7c7d85` without conflicts before final validation; the upstream delta is disjoint from this Search system slice.
- Regenerated and classified the split inventory: eight temporary system source-factory E2E edges, zero blocking E2E edges, and zero blocking test edges.
...
- Added the two Search implementation references only to the non-packable, carve-excluded system source factory and regenerated `inventory.json`.
...
- Added a source-free Search helper model test plus source-factory path tests; both remain in their owning Search/system targets and introduce no blocking test edge.
...
- `dotnet build api/tests/Concertable.System.E2E.Source/Concertable.System.E2E.Source.csproj --nologo` — passed with zero errors and two pre-existing sealed-constructor warnings.
...
- Package-only `dotnet build api/tests/Concertable.System.E2E.slnx --no-restore -p:UseLocalPlatformPackages=true -p:UseSystemSourceProjects=false ...` — passed with zero errors; existing MessagePack vulnerability and generated nullable warnings remain.
...
- System source-factory Search path unit tests — passed 2/2 against the exact-head local package feed.
- The successful carve contains no B2B, Customer, Payment, or Search service `src` directory and no `Concertable.System.E2E.Source` directory.
...
recorded in `reviews/Plan-RepoSplit-Stage4-System-E2E.md`: the full pass at `83c97871a` raised two findings,
...
- The original six source-mode inventory edges were already approved temporary system-factory edges. Search
  Web and Workers added two hidden source paths in a carve-retained service helper; this slice makes those two
  dependencies explicit and confines all eight to the carve-excluded system source factory.
