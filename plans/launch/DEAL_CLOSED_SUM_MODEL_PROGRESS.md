...
Genuinely heterogeneous operations use dedicated non-generic factories returning implementation unions:
Dunet on net10 and native unions on C# 15. Application acceptance is fixed in that classification,
not as a paid/simple command union.
...
`IDealUpdater`. Lifecycle accept/confirm/complete operations do not use this strategy factory: their
inputs, results, or capabilities are genuinely heterogeneous. Each uses a dedicated typed factory and
...
- Fixed acceptance as a dedicated-factory/implementation-union operation and reclassified
  confirm/complete the same way where landed APIs prove heterogeneous invocations.
...
- Application acceptance remains a dedicated `IAcceptFactory` returning a Dunet implementation union on
  net10 and a native implementation union on C# 15. It may not be reclassified as
  `IApplicationDealStrategyFactory<Accept>` or as an erased common step interface.
- The operation factory preserves the useful part of the former workflow: Deal-to-operation composition
  is declared once and the executor switches on acceptance kind. It rejects the former global bundle,
  capability-interface hierarchy, keyed service provider, and repeated four-Deal executor switch.
