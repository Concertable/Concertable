...
public sealed class VersusTermsEntity : DealTermsEntity
{
    private VersusTermsEntity() { }
    ...
    public static Result<VersusTermsEntity, ValidationErrors> Create(decimal guarantee, decimal artistDoorPercent, PaymentMethod paymentMethod)
    {
        var validation = Validate(guarantee, artistDoorPercent);
        return validation.Bind(() => Result.Success<VersusTermsEntity, ValidationErrors>(
            new VersusTermsEntity
            {
                Guarantee = guarantee,
