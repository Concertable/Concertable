...
- Worktree: `C:\Users\TommySeery\source\repos\Concertable.worktrees\Feature\platform_polyrepo-fullstack` — now sits on the **merged** Phase 3b branch. The next Phase 3 work (CSS `@source`, item 1 below) starts on a **fresh branch off current `origin/main`** — fetch first.
- Branch: `Feature/platform_polyrepo_carve-fe-ci` (Phase 3b) — **MERGED** into `main` as `1cbeb2175`. Stale/behind the new main tip; do not build further on it.
- PR: **Phase 3b PR [#389](https://github.com/Concertable/concertable/pull/389) MERGED** (`1cbeb2175`, 2026-08-06). First merge-queue run (31089695254) failed on a **UI E2E Playwright Response timeout** — an infra/host-load flake, not a regression (the diff is `test.yml`-only, incapable of affecting runtime UI); the queue auto-re-formed a fresh group (31089986474) that passed clean → merged. **Phase 3a [#378](https://github.com/Concertable/concertable/pull/378) MERGED** (`fba490e25`). Phase 2 [#360] MERGED (`a3f9535`); Phase 1 [#301] + review-fix [#319] merged earlier.
- Dependency/package gates: #389 is `test.yml`-only (0 `app/**` package change, 0 `api/**`) → **no FE republish and no backend platform-sync** (confirmed: latest `publish-fe-packages` run is still the #378 one [31045651556]; the open `chore/platform-sync-0.1.0-alpha.0.833` #396 is another PR's api sync, not ours, and is green-so-far). All five FE tiers remain at the #378 bare-export publish.
- Last reconciled: 2026-08-06 (resume) — re-synced #389 to current `main`, code-reviewed clean, resolved the GitHub auto-merge re-eval glitch with a one-time re-assert, rode out a UI-E2E flake (queue self-re-ran green), **#389 MERGED**. Next: the remaining Phase 3 items on a fresh branch off `origin/main`.
...
**Phase 3 remaining (the runtime/carve deferrals — see Next Steps):** carved-web CSS `@source` content strategy, mobile metro/nativewind/tailwind retarget (+ a mobile carve), and the ESLint import-boundary rule. None is blocked; each starts on a fresh branch off `origin/main`.

## Next Steps

Start each on a **fresh branch off current `origin/main`** (fetch first — the merged `carve-fe-ci` branch is stale). These are the Phase 3 runtime/carve deferrals; do them in dependency order.

1. **Carved-web CSS `@source` content strategy** — carve BUILD is green, but `@concertable/web`'s `index.css` `@source` globs point at sibling-surface source (`../../customer/src`, `../../b2b/*`) that resolve only in-monorepo; a carved surface generates its own classes (Tailwind v4 auto-detect) but NOT the shared tiers' (their class strings live in `node_modules/@concertable/*/dist`, which auto-detect excludes). A single relative `@source` set can't serve both layouts (monorepo `app/{shared,web/shared,…}` ≠ node_modules `@concertable/*`). Needs a cross-context strategy proven by a carved vite build's generated CSS; tier change → effective on republish. **NB the carve build passing ≠ CSS correct** — `carve-fe-web` checks the build compiles, not that Tailwind emitted the shared tiers' classes; verify the generated CSS, not just exit 0.
2. **Mobile metro/nativewind/tailwind retarget** off `../shared` onto `@concertable/mobile` (`watchFolders`, nativewind `input`, tailwind `content`), proven by `expo export` on the precompiled dist; then add `mobile/{customer,b2b}` to the `carve-fe-web` matrix (or a sibling `carve-fe-mobile`).
3. **FE import-boundary rule** — no ESLint/dependency-cruiser toolchain in `app/` yet; the carve CI is the primary structural boundary today (BE parity: carve = structural gate, build-time guard = fast second layer). Standing up ESLint `no-restricted-imports` across surfaces is a separate sub-project.
