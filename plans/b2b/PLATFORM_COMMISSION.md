...
its immutable SQL history if the ID and version are new. Reusing an existing ID or version with
different terms fails startup.

Changing the rate deploys one new ID, version and rate in Azure configuration. The previous Azure
...
or rate. Payment stores each deployed configuration once in SQL so historical bindings remain
...
- [x] Resolve review finding OWN1: keep one current revision in Azure configuration, insert each
  deployed revision once into immutable Payment SQL history, and persist only its foreign key on bindings.
- [x] Verify OWN1 on current `origin/main`: Payment unit tests (135 passed),
...
- reusing an ID or version with different terms fails startup, and referenced SQL revisions remain
