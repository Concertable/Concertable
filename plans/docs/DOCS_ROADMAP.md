...
| [ ] | `docs/polyrepo-ready` | Finish the split the polyrepo cut requires. **Phase 1 + N1–N7a all MERGED** (plan process, all 28 workflow skills across six families, the route-table convention + generator, `api/AGENTS.md`, `api/ARCHITECTURE.md` + `MICROSERVICES_ARCHITECTURE.md`, root `AGENTS.md`, `docs/`, and `plans/AGENTS.md` — each re-homed to the plugins, values left behind). **N8 evidence merged (#764)** (carved Payment table: routes fire, all 37 skills resolve; surfaced one real defect — generator drift). That drift is **fixed** (agent-standards #34 repointed the CANONICAL to `\.ArchitectureTests`; the live route table was regenerated so `gen --check` is clean), the two `PLANS.md` follow-ups landed in the same PR, and `auto-memory` is homed in the agent-process plugin. Remaining before close-out: delete the spent review records + this plan/ledger. Gated/separate: N7b (plan documents, §4c/§6) and the cut itself | docs/guidance-restructure |
