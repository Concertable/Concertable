...
- Branch: `Feature/platform_polyrepo_mobile-retarget` @ `92d48f6db`, off `origin/main` @ `529dba9dd` (0 behind). Carries the mobile bundler retarget + carve-fe mobile wiring.
- PR: **mobile-retarget PR [#413](https://github.com/Concertable/concertable/pull/413) OPEN** (base `main`, no skip labels → full E2E; `run_fe` runs the renamed `carve-fe` matrix incl. the two new mobile surfaces). Prior: carved-web CSS **[#405](https://github.com/Concertable/concertable/pull/405) MERGED** (`d9c62e2c5`) + closeout [#411]; Phase 3b [#389] (`1cbeb2175`) + [#398]; Phase 3a [#378] (`fba490e25`); Phase 2 [#360] (`a3f9535`); Phase 1 [#301]+[#319].
- Dependency/package gates: on #413 merge, `publish-fe-packages` republishes the 5 tiers (no tier *source* changed — surfaces + carve script + `test.yml` only — so a no-op-content republish; harmless). No `api/**` → no backend platform-sync.
- Last reconciled: 2026-08-07 — mobile retarget **implemented + committed (`92d48f6db`), PR #413 open**. Local `expo export`/carve pre-verification was **blocked by the Windows AV file-locking** that stalls every heavy `app/` install/`rm` here (documented; Phase 3a/3b hit the same and let CI validate). The Linux `carve-fe` mobile `expo export` job on #413 is the definitive proof. Remaining after #413: Phase 3 ESLint import-boundary rule.
...
**Mobile metro/nativewind/tailwind retarget — IMPLEMENTED on `Feature/platform_polyrepo_mobile-retarget` (`92d48f6db`), PR #413 OPEN.** Both `app/mobile/{customer,b2b}` `metro.config.js` (`watchFolders` + NativeWind `input`) and `tailwind.config.js` (`content`) now `require.resolve("@concertable/mobile/…")` and scan the package's compiled `dist/**/*.js`, instead of the `../shared` sibling that doesn't exist when carved — the mobile counterpart of the web #405 fix. `@concertable/mobile` is the only className-bearing mobile tier (208 `className=` vs 0 in the customer/shared logic-only tiers). `carve-fe.mjs`'s mobile branch now runs `expo export` after `tsc --noEmit` (tsc can't see the bundler config), and `test.yml`'s carve job was renamed `carve-fe-web` → `carve-fe` with `mobile/{customer,b2b}` added to the matrix + `ci-complete`.

**Phase 3 remaining after #413:** the ESLint import-boundary rule (item 2 below).

## Next Steps

Mobile retarget is delivered pending PR #413 merge + green `carve-fe`. **Follow #413** to merged + green (the mobile `expo export` carve is the proof; if it goes red, debug on-branch and push — do not weaken the gate). Then, on a **fresh branch off current `origin/main`** (fetch first):

1. **FE import-boundary rule** — no ESLint/dependency-cruiser toolchain in `app/` yet; the carve CI is the primary structural boundary today (BE parity: carve = structural gate, build-time guard = fast second layer). Standing up ESLint `no-restricted-imports` across surfaces is a separate sub-project.
