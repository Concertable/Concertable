...
1. **BLOCKED — do not merge PR #933.** Owner decision, 2026-09-04: Payment's published surface still carries consumer-domain vocabulary (`Manager*` across the service, client, error families and the gRPC messages; `BookingId` as an indexed `int` column on `Escrows`, `FinancialOperations`, `LedgerTransactions` and `SettlementTransactions`, and on the wire), and the agnosticism must be settled before this producer publishes. Resume condition: the owner picks the route — fold the agnostic surface into this PR as one clean break, or publish as-is and clean up in the legacy cull — and its plan is authored. Sequencing note: no consumer is on the reference surface yet (B2B adopts it on PR #633 after publish; Customer has not started), so the compatibility this PR currently preserves is preserved for nobody; a clean break here costs each consumer one migration instead of two.
2. Dispatch the downstream handoffs at the publish gate: PR #633 (`DEAL_LIFECYCLE_OWNERSHIP_PROGRESS.md`) advances its Payment pins against the published packages, revalidates, goes ready, and merges — and its owner registers the `PAYMENT_LEGACY_CULL_PROGRESS.md` downstream handoff in its own ledger; surface the start prompt for `CUSTOMER_PAYMENT_REFERENCE_PROGRESS.md` in the same dispatch.
...
5. Customer is a second raw-identifier consumer with the same flaw (`TicketPurchaseParams.PaymentMethodId` → `Ticket.Infrastructure/Services/TicketService.cs:76` → `ICustomerPaymentClient.PayAsync(..., paymentMethodId)`, relayed from the customer web and mobile surfaces via `app/customer/shared/src/features/tickets/types.ts`). Its migration onto the reference surface is its own follow-on consumer plan — same package gate as B2B, independent of PR #633, on-session `Payment`-kind sessions rather than off-session commitments, and saved-card selection becomes a Payment surface per `PAYMENT_BOUNDARY_DECISION.md` §1. The Payment legacy cull (decision doc §7 step 5) is gated on **both** consumer migrations: it must not fire while Customer still calls the raw-identifier APIs.
...
- `plans/launch/CUSTOMER_PAYMENT_REFERENCE_PROGRESS.md` — no worktree yet. Gate: the same package publish; surface its start prompt at the Next Steps item 2 dispatch.
- `plans/launch/PAYMENT_LEGACY_CULL_PROGRESS.md` — no worktree yet. Gate: both consumer migrations terminal (the B2B migration riding PR #633 and the Customer payment-reference migration); dispatch it only when its ledger's Resume-when condition holds.

## Decisions, discoveries, blockers, and deviations

- **Payment is not yet consumer-domain agnostic, and this plan's own surface is not the whole gap.** `plans/launch/PAYMENT_LEGACY_CULL_PLAN.md` states the right invariant ("no consumer-domain noun on any published Payment contract") but its Delivery steps miss the two largest violations: the `Manager*` family (a B2B role noun — payments has payers and payees, not managers) and `BookingId` (a foreign service's `int` primary key, persisted and indexed inside Payment's schema). `BookingId` is an older, non-agnostic form of the `ConsumerCorrelation` this plan introduced. Legitimately Payment's own and out of scope for any rename: `Escrow`, `Commission`, `Settlement`, `PayoutAccount`, `Ledger`, and the opaque `payerId`/`payeeId` owner ids.
