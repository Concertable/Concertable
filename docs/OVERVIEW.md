...
- **Backend** — .NET services split across `api/Concertable.B2B/` and `api/Concertable.Customer/`, each a modular host with modules under its own `Modules/` folder (Artist, Venue, Concert, Contract, Payment, Notification, Search, Identity/User, etc.). Cross-module calls go through `IXModule` facades in `Module.Contracts`; rules in [`CONVENTIONS.md`](../api/agents/CONVENTIONS.md).
