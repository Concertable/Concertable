...
**If you notice a smell or bug you can't address in the current PR — a pre-existing one, or something out of scope — log it as tech debt; noticing it and moving on without a record is the failure.** Out-of-scope is a reason to *log*, never a reason to *skip*: whether it's a latent bug, a hacky workaround, an inconsistency, or debt a prior change introduced, it gets a line. **And if the closest area has no `TECH_DEBT.md`, create one** — a missing file is never a reason to skip logging (mirror the format of any sibling `TECH_DEBT.md`).
