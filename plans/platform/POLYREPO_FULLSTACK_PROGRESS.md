...
- PR: **mobile-retarget PR [#413](https://github.com/Concertable/concertable/pull/413) OPEN/CLEAN at reviewed head `a8296d51b`; green but unadmitted** — auto-merge has been enabled since `2026-08-07T14:27:09Z`, but the PR has no merge-queue entry and no merge-group run. This is the documented GitHub re-evaluation glitch, not a test failure; one disable/re-enable is the resolved remedy. Prior: carved-web CSS **[#405](https://github.com/Concertable/concertable/pull/405) MERGED** (`d9c62e2c5`) + [#411]; Phase 3b [#389] (`1cbeb2175`) + [#398]; Phase 3a [#378] (`fba490e25`); Phase 2 [#360] (`a3f9535`); Phase 1 [#301]+[#319].
...
- Last reconciled: 2026-08-07 — PR #413 source head `a8296d51b` is review-clean, current with `origin/main`, and green across all terminal PR-head checks, but GitHub never admitted its long-enabled auto-merge request to the queue. There is no merge-group failure. Next: re-assert auto-merge once, verify admission, and follow full merge-group E2E to merge; then republish and turn on the mobile carve gate.
