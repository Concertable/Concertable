...
4. Without a plan reference, inspect the current worktree's branch, modified plan files, branch-specific commits, PR, and plan names. Exclude `AGENTS.md` and `*_PROGRESS.md` from plan candidates. Select the plan only when the evidence identifies it. If multiple plans remain equally plausible, report the candidates and ask for the plan path instead of inventing a status.
