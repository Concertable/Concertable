...
2. **Genuinely heterogeneous input, output, or capability:** give the operation a dedicated non-generic
   factory, return a Dunet union over the concrete DI implementations on net10 and a native C# 15 union
   over those implementations on .NET 11, then match by operation case. Several Deal cases may map to
   the same implementation. Acceptance, confirmation, and completion candidates belong here; they are
   not examples for the common-interface strategy factory.
...
No current Booking or Concert executor/step family is approved merely because an erased interface gives
four implementations the same base-typed signature. Eligibility requires one genuinely substitutable
invocation without case-specific casts or hidden capability differences. Do not manufacture or preserve
a common interface for acceptance, confirmation, completion, or another heterogeneous lifecycle
operation merely to use the strategy factory.
...
### 4.8 Dedicated heterogeneous-operation factories

A heterogeneous operation uses the same generated catalog and registration infrastructure but a
...
{
    internal partial record CaptureEscrow(CaptureEscrowAccept Value);
    internal partial record Paid(PaidAccept Value);
    internal partial record DepositEscrow(DepositEscrowAccept Value);
}

internal interface IAcceptFactory
{
    Accept Create(IDeal deal);
}
```

The generated `AcceptFactory` uses typed constructor injection for the three concrete implementations
and owns the only Deal-to-operation switch. Its module catalog deliberately maps both DoorSplit and
Versus to `PaidAccept`; this is an alias, not missing coverage. It has an explicit unknown-`IDeal`
fallback because net10 cannot close the hierarchy. Dunet's generated `.Match(...)` requires every
operation case at the consumer call site.
...
The C# 15 surface keeps `IAcceptFactory` and the same composition but replaces the wrapper:

```csharp
internal union Accept(
    CaptureEscrowAccept,
    PaidAccept,
    DepositEscrowAccept);
...
internal interface IAcceptFactory
{
    Accept Create(Deal deal);
}
```

The generated factory switch is exhaustive over `closed Deal`; the consuming switch is exhaustive over
`Accept`. This is the intended two-stage macro: the module declares Deal-to-operation composition once,
the generator expands it once, and every consumer branches on semantic operation shape rather than
repeating Deal cases. The factory name is the noun it returns (`IAcceptFactory`), not
`IAcceptStepFactory`; `Step` is obsolete workflow vocabulary at this boundary.

Application owns one Infrastructure generation anchor and one registration entry point for both kinds
of output. It emits invariant generic strategy factories for honest families and dedicated non-generic
operation factories for heterogeneous families. Booking and Concert add equivalent module anchors only
for catalogs they actually own. This reuses generator machinery without pretending the contracts are
the same.
...
| Application acceptance | Heterogeneous operation capabilities | Dedicated `IAcceptFactory`; Dunet union over concrete implementations; match by acceptance kind | Same factory semantics; native implementation union and exhaustive matches |
| Booking confirmation | Heterogeneous accepted/financial facts and effects | Dedicated factory plus Dunet implementation/value unions after landed-API inventory | Native unions and exhaustive matches |
...
## 7. Acceptance classification is fixed

Acceptance is permanently classified as a heterogeneous operation, not as an honest common-interface
strategy family. Its factory returns the closed set of concrete acceptance implementations selected by
the module catalog; it does not return a command/request union containing `ApplicationEntity`.

The exact implementation cases must be reconciled against the landed module APIs before code is moved.
The currently approved semantic set is capture-escrow acceptance, paid acceptance, and deposit-escrow
acceptance, with deliberate many-Deal-to-one-operation mappings where applicable. If the lifecycle split
proves a different operation belongs to Booking rather than Application, ownership and naming follow the
landed module; the classification and factory/union structure do not change.

This shape may not be reclassified as `IApplicationDealStrategyFactory<Accept>`, an erased common step
interface, a four-Deal-case executor switch, an enum, nullable context wrapper, keyed service provider,
or global workflow hierarchy. The only switches are the generated Deal-to-operation mapping and the
consumer's exhaustive operation-union match.
...
4. deliberate mapping in every affected dedicated operation factory and new semantics in every affected
   native/Dunet union or closed data value;
...
- native unions: compiler-exhaustive operation match fails when a genuinely new case is introduced;
- Dunet unions: generated `.Match(...)` calls require a handler for every declared operation case;
