...
- **Check for commit-plan artifacts** before inventing structure: `*COMMIT_PLAN*.md`, notes in AGENTS.md, TODO/plan files referenced in memory. If one exists, **verify its claims against the actual tree** — plans go stale (dirs it says were moved may have been moved back). Follow what still holds; tell the user what didn't.
...
- **No AI attribution**: never add `Co-Authored-By: Codex`, `Generated with Codex`, or similar trailers.
