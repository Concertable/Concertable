...
| [ ] | `docs/polyrepo-ready` | Finish the split the polyrepo cut requires. **Phase 1 + N1–N7a all MERGED** (plan process, all 28 workflow skills across six families, the route-table convention + generator, `api/AGENTS.md`, `api/ARCHITECTURE.md` + `MICROSERVICES_ARCHITECTURE.md`, root `AGENTS.md`, `docs/`, and `plans/AGENTS.md` — each re-homed to the plugins, values left behind). **N8 evidence recorded** (carved Payment table: routes fire, all 37 skills resolve; surfaced one real defect — the generator CANONICAL still emits `\.CompositionTests` while the live table was hand-repointed to `\.ArchitectureTests`, so `gen` no longer reproduces the live table). Remaining before close-out: fix that generator drift + two `PLANS.md` follow-ups (one consolidated agent-standards PR), give `auto-memory` a durable home, delete the spent review-sweep file. Gated/separate: N7b (plan documents, §4c/§6) and the cut itself | docs/guidance-restructure |
