...
| Query parameters, route construction, headers, JSON/multipart encoding | Keep in `api/xApi.ts`; extract an API-local named helper only when the encoding itself is complex or reused. |
...
| `app/web/b2b/shared/src/features/organizations/components/OrganizationForm.tsx` | `initialBuffer` flattens an `Organization` read into editable fields and defaults absent compliance data. | Move `OrganizationBuffer` to feature `types.ts`; add `Organization.toBuffer`; initialize component state with it. |
...
| `app/customer/shared/src/features/preferences/api/preferenceApi.ts`, `hooks/usePreferenceQuery.ts`, and mobile `PreferencesScreen.tsx` | Update accepts a full `Preference` read and the screen reconstructs server-owned `id`/`user`; create drops selected genres. | Replace create/update mutation inputs with one slim `PreferenceRequest`; keep route `id` as a function argument; validate the two-field editable buffer and pass parsed data directly. Add no companion because the parsed buffer and request are identical. |
| `app/shared/src/features/concerts/hooks/useMyConcert.ts` and the concert edit store | A full `Concert` read is copied as editable state and silently stripped to an update request by Zod. | Introduce a writable `ConcertBuffer`; add `Concert.toBuffer`; store only the buffer; parse it directly as `UpdateConcertRequest`. Do not add a second request mapper when parsed data already has the request shape. |
| `app/shared/src/features/artists/api/artistApi.ts`, `hooks/useMyArtist.ts`, and artist edit store | Create types live in the API module; update accepts a full `Artist` read, then manually encodes selected fields as multipart. | Move slim create/update request interfaces to feature `types.ts`; add an editable buffer and `Artist.toBuffer`; parse before mutation. Keep `FormData` construction in `artistApi.ts` as transport encoding and type the API with the slim request. |
| `app/shared/src/features/venues/api/venueApi.ts`, `hooks/useMyVenue.ts`, and venue edit store | Update accepts a full `Venue` read, then manually encodes selected fields as multipart. | Apply the Artist shape symmetrically: slim request interfaces, `Venue.toBuffer`, parsed mutation input, API-local `FormData` encoding. |
| `app/shared/src/features/search/api/headerApi.ts` | `SearchFilters` is renamed and combined into HTTP query parameters. | Keep it in `headerApi.ts`; name/type an API-local `toSearchParams` helper if extraction improves the method. Do not put transport parameters on `SearchFilters`. |
| `app/shared/src/features/artists/api/artistApi.ts` and `venues/api/venueApi.ts` | Domain/request fields become PascalCase multipart entries and indexed genre keys. | Keep in each API module. These are wire encoders, not domain companions. |
| `app/web/shared/src/features/dashboard/components/MonthlyRevenueChart.tsx` | Revenue points become Recharts rows with formatted months and major currency units. | Keep a named projection beside the chart. It is a presentation model, not domain behaviour. |
...
1. Add the finalized companion convention and classification table to
   `app/agents/CODE_PATTERNS.md`, integrating with the post-#637 structure rather than restoring old
   prose.
...
npm -w @concertable/mobile-customer run build
npm -w @concertable/mobile-b2b run build
git diff --check
```
...
- Pure companion tests live beside the owning feature as `types.test.ts` unless that feature already
  has a more specific established test location.
