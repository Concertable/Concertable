...
- PR: **mobile-retarget PR [#413](https://github.com/Concertable/concertable/pull/413) OPEN/CLEAN at reviewed head `a8296d51b`; auto-merge enabled** — publish-first (retarget + tier asset fix; web-only carve matrix, full E2E). All PR-head build, web-carve, backend-carve, unit, and integration checks are green; PR-level E2E is expectedly skipped pending merge-queue execution. Prior: carved-web CSS **[#405](https://github.com/Concertable/concertable/pull/405) MERGED** (`d9c62e2c5`) + [#411]; Phase 3b [#389] (`1cbeb2175`) + [#398]; Phase 3a [#378] (`fba490e25`); Phase 2 [#360] (`a3f9535`); Phase 1 [#301]+[#319].
...
- Last reconciled: 2026-08-07 — PR #413 source head `a8296d51b` is review-clean, current with `origin/main`, and green across all terminal PR-head checks; auto-merge is enabled. The CI `carve-fe` mobile `expo export` proved the retarget resolves the tier from the feed (bundled **4325 modules**) before hitting a **pre-existing** tier bug: `@concertable/mobile`'s `Logo` requires `assets/brand/logo*.png` that the package never shipped and that lived outside the package — fixed here by moving `brand/` into the tier. Local pre-verification was impossible (Windows AV blocks all heavy `app/` npm). Next: verify queue admission and follow #413 through full merge-group E2E to merge, then republish and turn on the mobile carve gate.
