...
Replace the flat-fee setting with a validated append-only Payment configuration catalog:
...
deal-type selector, expiry or mutable status. The catalog binds through normal .NET configuration:
local appsettings and deployed Azure App Configuration expose the same shape. Payment validates the
complete catalog on startup and holds it through `IOptions`; commission rates do not hot-reload inside
a running process.
...
from the in-memory catalog rather than the current revision.
...
CommissionConfigurationId  required reference into the immutable Payment configuration catalog
...
- the current configuration ID must resolve to one validated catalog revision;
- duplicate IDs or versions and invalid terms fail startup, and referenced revisions remain in the
  append-only configuration catalog;
