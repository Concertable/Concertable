...
- **2026-07-01** — **Revenue model decided: % commission on the settlement transaction — ~5% headline, with a small minimum floor.** The last open "decide-first" gate (§9), now closed. Rationale: the locked "fee rides the settlement transaction" principle (2026-06-22) structurally favours a %; a % that only bills on *completed* bookings sells with zero traction, whereas an access subscription (GigPig: £150–250/mo tiers + ~£10–15/gig PAYG) is viable only once you have an established high-frequency-venue base. Competitor anchor from web research: **GigXchange** charges **0–8%** on settlement (0% if settled offline — a leak we don't share, since DoorSplit/Versus *must* route through us to settle at all); traditional agencies (Encore, Alive) take **~20%**; Stripe cost is ~1.5%+20p — so ~5% is competitive with healthy margin, and the floor stops sub-threshold gigs losing money on fees. Exact % is revisitable at v1.1; the *shape* (% on settlement + floor) is what unblocks the pricing-transparency UI, the solicitor T&Cs brief, and per-contract VAT calc. Subscription parked as a possible later add-on, not v1. Sources: GigPig venue pricing page; GigXchange 2026 UK-platform comparison.
