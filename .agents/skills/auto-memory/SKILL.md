---
name: auto-memory
description: Toggle Codex's auto-memory feature on or off for this project by flipping the `autoMemoryEnabled` key in `.Codex/settings.local.json`. Pure toggle — no arguments. Use whenever the user wants to turn memory on/off, disable memory, enable memory, or stop/start memory recall for the project.
---
...
- This only controls auto-memory (recall + saving of `MEMORY.md` / `memory/` files). It does
  **not** affect `AGENTS.md` project instructions, which always load.
