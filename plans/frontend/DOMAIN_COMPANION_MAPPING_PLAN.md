...
| `app/web/b2b/shared/src/features/concerts/hooks/useMyConcert.ts` and `app/shared/src/features/concerts/store/useConcertStore.ts`       | A full `Concert` read is copied as editable state and silently stripped to an update request by Zod.                          | Define `ConcertBuffer`, `UpdateConcertRequest`, and `Concert.toBuffer` in `app/shared/src/features/concerts/types.ts`; store only the buffer; bind the existing schema to the request interface and pass parsed data directly. Do not add a second request mapper.                                                                          |
...
| Artist, Venue, and Concert editor pages/components                 | Public stores expose draft fields and transitions directly; several pages combine those stores with existing workflow hooks.                     | During the existing buffer phases, make `useMyArtist`, `useMyVenue`, and `useMyConcert` return their draft and named editor actions; add create workflow hooks where needed; pass image callbacks into Hero components. Remove all editor-store barrel exports and component imports. |
...
4. Replace the full-Concert edit copy with `ConcertBuffer`, `Concert.toBuffer`, and direct parsed
   request mutation.
5. Expand `useMyConcert` to expose its draft and editor actions, then remove `useConcertStore` from
   barrels and components.
6. Test semantic normalization, route/body separation, create/update parity, buffer initialization,
...
4. Keep PascalCase multipart keys, optional image handling, and indexed genre fields inside the API
   modules.
5. Make the Artist and Venue facade hooks own all editor transitions; add create facades, pass image
   callbacks into Hero components, and remove public store exports and component/store imports.
6. Test the pure buffer conversions and API-level multipart encoding separately. Do not test browser
