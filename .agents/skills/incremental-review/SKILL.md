...
Default: `reviews/<branch-slug>.md` (branch `/` → `-`). If the user named a file, or a legacy review file exists for this branch (e.g. `plans/PR_FEEDBACK.md`), use that. The file must already exist — this skill is incremental. If none exists, tell the user to run `the `code-review` skill` first.
...
Read `.agents/skillsthe `code-review` skill/SKILL.md` and **follow it from Step 3 onward verbatim**, with exactly one substitution: the review range is `<start>..HEAD` (the watermark SHA from Step 1/2), not the merge-base.
