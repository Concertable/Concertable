...
- [ ] 🟡 **Organiser direct offers — the second booking entry direction** `launch/booking-entry-direct-offers` — design pass complete 2026-09-07, read against `origin/main` at the PR #633 merge. Venues create opportunities and artists apply; an organiser must also be able to select an artist and send an offer to accept, decline or counter. The feared entry-route × deal-type matrix is one hardcoded coincidence: `ApplicationCheckoutExtensions` reads as a deal-type rule and is actually `Payer == Artist`, and the payer axis already exists as `IDealPayeeResolver`'s two leaves across four keys. The payer must be present when their instrument is collected, so payer-last authorises on-session (`CaptureEscrow`) and payer-first leaves a method for an off-session move (`DepositEscrow`) — both new offer cells reuse operations that already exist, and Payment needs no change because its escrow commands take plain `PayerId`/`PayeeId`. Modelling: `ApplicationEntity` becomes a negotiation thread and an append-only `ProposalEntity` carries the versions, which nets **−2** nullable fields, keeps both unique indexes, needs no new lifecycle state (one `Counter` self-loop), and gives counteroffers the versioning the product doc requires. Opportunity gains one non-null admission enum and no strategy. No `OpportunityWorkflow` — separate `DbContext`s and a read-only `IOpportunityModule` make the composition the client's. No published contract changes, so one PR. `launch/operation-claims-and-attempts` is a rebase collision on three files, not a dependency. Plan: [BOOKING_ENTRY_DIRECT_OFFERS_PLAN.md](BOOKING_ENTRY_DIRECT_OFFERS_PLAN.md).
