...
- **Frontend code conventions (all of `app/`)** — naming, contract types, casing, null-vs-undefined,
  query/mutation shape, stores, form buffers: [`app/agents/CODE_CONVENTIONS.md`](./app/agents/CODE_CONVENTIONS.md).
  The frontend counterpart of `api/agents/CODE_CONVENTIONS.md` — read it before writing any `app/` code.
