...
public sealed class ContractApiTests : IAsyncLifetime
{
    ...
    public async Task Accept_ShouldSnapshotContract_ThatSurvivesContractEdit_ForFlatFee()
    {
        ...
        var opportunityId = await CreateOpportunityAsync(new FlatFeeTerms { PaymentMethod = PaymentMethod.Transfer, Fee = 500m });
        ...
        await UpdateDealAsync(opportunityId, new FlatFeeTerms { PaymentMethod = PaymentMethod.Cash, Fee = 999m });
    ...
    {
        // Arrange
        var opportunityId = await CreateOpportunityAsync(new DoorSplitTerms { PaymentMethod = PaymentMethod.Cash, ArtistDoorPercent = 70m });
        var applicationId = await ApplyAsync(opportunityId);
        var venueClient = fixture.CreateClient(fixture.SeedState.VenueManager1);
        ...
        // Act
        await UpdateDealAsync(opportunityId, new DoorSplitTerms { PaymentMethod = PaymentMethod.Cash, ArtistDoorPercent = 15m });
    ...
    {
        // Arrange
        var opportunityId = await CreateOpportunityAsync(new VersusTerms { PaymentMethod = PaymentMethod.Cash, Guarantee = 200m, ArtistDoorPercent = 60m });
        var applicationId = await ApplyAsync(opportunityId);
        var venueClient = fixture.CreateClient(fixture.SeedState.VenueManager1);
        ...
        // Act
        await UpdateDealAsync(opportunityId, new VersusTerms { PaymentMethod = PaymentMethod.Cash, Guarantee = 999m, ArtistDoorPercent = 10m });
    ...
    public async Task Accept_ShouldSnapshotContract_ThatSurvivesContractEdit_ForVenueHire()
    {
        ...
        var opportunityId = await CreateOpportunityAsync(new VenueHireTerms { PaymentMethod = PaymentMethod.Cash, HireFee = 250m });
        ...
        // Act
        await UpdateDealAsync(opportunityId, new VenueHireTerms { PaymentMethod = PaymentMethod.Cash, HireFee = 999m });
    ...
    public async Task Accept_ShouldReturn409_WhenTermsChangedSinceApply()
    {
        ...
        var opportunityId = await CreateOpportunityAsync(new FlatFeeTerms { PaymentMethod = PaymentMethod.Transfer, Fee = 500m });
        var applicationId = await ApplyAsync(opportunityId);
        await UpdateDealAsync(opportunityId, new FlatFeeTerms { PaymentMethod = PaymentMethod.Transfer, Fee = 999m });
        var venueClient = fixture.CreateClient(fixture.SeedState.VenueManager1);
        await venueClient.PostAsync($"/api/Application/{applicationId}/checkout");
    ...
    private async Task<int> CreateOpportunityAsync(IDealTerms terms)
    {
        var venueClient = fixture.CreateClient(fixture.SeedState.VenueManager1);
    ...
    private async Task UpdateDealAsync(int opportunityId, IDealTerms desired)
    {
        var opportunity = await fixture.ConcertReads.Set<OpportunityEntity>()
            .FirstAsync(o => o.Id == opportunityId);

        using var scope = fixture.Services.CreateScope();
        var contracts = scope.ServiceProvider.GetRequiredService<IDealTermsModule>();
        var update = await contracts.UpdateAsync(opportunity.DealTermsId, desired);
        Assert.True(update.IsSuccess);

        var updatedOption = await contracts.GetByIdAsync(opportunity.DealTermsId);
        Assert.True(updatedOption.TryGetValue(out var updated));
        desired.Id = opportunity.DealTermsId;
